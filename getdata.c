...
{
	if(!F) return -1;
	return F->n_raw + F->n_polynom + F->n_lincom + F->n_linterp + F->n_multiply + F->n_mplex + F->n_bit;
}
		...
		char ** name, char * field_code)
{
	if(ind < F->n_raw) {
		*category = "raw";
		*name = F->rawEntries[ind].field;
		*field_code = F->rawEntries[ind].type;
		return 1;
	}
	ind -= F->n_raw;
	/* For the others, getting the native data type is cumbersome, so
	 * we just guess at 'd' */
	if(ind < F->n_polynom) {
		*category = "polynom";
		*name = F->polynomEntries[ind].field;
		*field_code = 'd';
		return 1;
	}
	ind -= F->n_polynom;
	if(ind < F->n_lincom) {
		*category = "lincom";
		*name = F->lincomEntries[ind].field;
		*field_code = 'd';
		return 1;
	}
	ind -= F->n_lincom;
	if(ind < F->n_linterp) {
		*category = "linterp";
		*name = F->linterpEntries[ind].field;
		*field_code = 'd';
		return 1;
	}
	ind -= F->n_linterp;
	if(ind < F->n_multiply) {
		*category = "multiply";
		*name = F->multiplyEntries[ind].field;
		*field_code = 'd';
		return 1;
	}
	ind -= F->n_multiply;
